...
func GetRoutes() *mux.Router {
	...
	assetManagerRoutes := protectedRoutes.PathPrefix("/inventory").Subrouter()
	assetManagerRoutes.Use(middlewares.RequireRole(models.AssetManagerRole, models.AdminRole))
	//post methods
	assetManagerRoutes.HandleFunc("/asset", handler.AddNewAssetWithConfig).Methods("POST")
	assetManagerRoutes.HandleFunc("/asset/assign", handler.AssignAssetToUser).Methods("POST")
	assetManagerRoutes.HandleFunc("/asset/unassign", handler.RetrieveAsset).Methods("POST")
	assetManagerRoutes.HandleFunc("/asset/service/send", handler.SendAssetToService).Methods("POST")
	assetManagerRoutes.HandleFunc("/asset/service/received", handler.ReceivedFromService).Methods("POST")

	//put methods
	assetManagerRoutes.HandleFunc("/asset/update", handler.UpdateAssetWithConfigHandler).Methods("PUT")

	//get methods
	assetManagerRoutes.HandleFunc("/assets", handler.GetAllAssetsWithFilters).Methods("GET")
	assetManagerRoutes.HandleFunc("/asset/timeline", handler.GetAssetTimeline).Methods("GET")

	//delete methods
	assetManagerRoutes.HandleFunc("/asset/remove", handler.DeleteAsset).Methods("DELETE")
	...
	employeeManagerRoutes.HandleFunc("/timeline", handler.GetEmployeeTimeline).Methods("GET")
